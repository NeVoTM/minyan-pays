...
|----------|-------------|
| **Deploy build / pre-deploy** | API **build** runs **`npx prisma db push --accept-data-loss`** before **`npm run build`** so schema sync succeeds even if the dashboard **Pre-deploy** command is missing or stale (`DATABASE_URL` is available during Render builds). **Pre-deploy** still runs **`npx prisma db push --accept-data-loss && npm run db:seed`** (second push is a no-op; seed skips if orgs exist). Copy both lines from [`render.yaml`](../render.yaml) into the Render dashboard if the service was not created from the Blueprint. Prefer **`prisma migrate deploy`** with a baselined history once you move off ad-hoc `db push` for production. |
