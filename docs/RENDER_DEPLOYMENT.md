...
|----------|-------------|
| **Deploy build / pre-deploy** | API **`npm run build`** runs [`apps/api/scripts/render-build.mjs`](../apps/api/scripts/render-build.mjs): when **`RENDER=true`** (Render’s build), it runs **`prisma db push --accept-data-loss`**, **`tsc`**, and **`npm run db:seed`**; locally it runs **`tsc` only** (no DB). Blueprint **`preDeployCommand`** is **`true`** (no-op) so a stale dashboard line like `npx prisma db push` (without `--accept-data-loss`) cannot fail the deploy. **If your API service still fails in “Pre-deploy”:** open Render → service → **Settings** → **Pre-deploy Command** and replace it with **`true`** or **delete it entirely**, then save and redeploy. Prefer **`prisma migrate deploy`** with a baselined history once you move off ad-hoc `db push` for production. |
