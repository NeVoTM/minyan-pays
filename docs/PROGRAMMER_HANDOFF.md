...
- **Security — `apps/api/src/routes/auth.ts`:** **`POST /api/auth/admin`** requires **`ADMIN_PASSWORD`** (env). **`POST /api/auth/rabbi`** verifies **`Organization.rabbiPasswordHash`** when set, otherwise **`RABBI_PASSWORD`** (env). **`POST /api/auth/member`** requires **`pin`** and compares **`User.pinHash`** (bcrypt). JWTs use **`24h`** expiry; payload includes **`organizationId`** and role. **Cross-tenant:** protected routes must continue to validate org + role (audit middleware vs resources as you extend APIs).
