...
/** Whitelist of paths a SHAMOSH-role token may call (lookup is path-prefix based). */
const SHAMOSH_ALLOW: ReadonlyArray<{ method: string; matches: (p: string) => boolean }> = [
  { method: "GET", matches: (p) => p === "/me" },
  { method: "GET", matches: (p) => p === "/session/today" },
  { method: "POST", matches: (p) => /^\/attendance\/[^/]+\/confirm$/.test(p) },
  { method: "POST", matches: (p) => /^\/attendance\/[^/]+\/reject$/.test(p) },
  { method: "POST", matches: (p) => /^\/attendance\/[^/]+\/cancel$/.test(p) },
  { method: "GET", matches: (p) => p === "/settings" },
];
...
rabbiRouter.use((req, res, next) => {
  ...
  const ok = SHAMOSH_ALLOW.some(
    (r) => r.method === req.method && r.matches(req.path)
  );
  if (!ok) {
    res.status(403).json({
      error:
        "Shamoshim may only review and approve today's check-ins.",
    });
    return;
  }
  next();
...
rabbiRouter.get("/me", async (req, res) => {
  ...
  if (a.rabbiKind === "SHAMOSH" && a.shamoshId) {
    const s = await prisma.shamosh.findUnique({
      where: { id: a.shamoshId },
      select: { id: true, name: true, rabbiId: true },
    });
    res.json({
      role: "RABBI",
      rabbiKind: "SHAMOSH",
      shamoshId: a.shamoshId,
      rabbiId: s?.rabbiId ?? null,
  ...
  if ((a.rabbiKind === "MAIN" || a.rabbiKind === "ADDITIONAL") && a.rabbiId) {
    const r = await prisma.rabbi.findUnique({
      where: { id: a.rabbiId },
      select: { id: true, name: true, isMain: true },
    });
    res.json({
      role: "RABBI",
      rabbiKind: a.rabbiKind,
      rabbiId: a.rabbiId,
      displayName: r?.name ?? (a.rabbiKind === "MAIN" ? "Main rabbi" : "Rabbi"),
      isMain: r?.isMain === true,
      canManageShamoshim: true,
      canManageRabbis: r?.isMain === true,
      isShamoshView: a.rabbiKind === "ADDITIONAL",
    });
    return;
  }
  res.json({
    role: "RABBI",
    rabbiKind: a.rabbiKind ?? "LEGACY",
    displayName: "Rabbi",
    canManageShamoshim: true,
    canManageRabbis: true,
    isShamoshView: false,
  });
});
...
async function resolveShamoshPassword(
  d: z.infer<typeof shamoshUpsertSchema>,
  ...
  if (d.password === null || d.password === "") return { plain: null, hash: null };
  const trimmed = d.password.trim();
  if (!isValidRabbiPassword(trimmed)) {
    throw new Error("Password must be exactly 8 letters/numbers (a–z, A–Z, 0–9).");
  }
  await assertRabbiPasswordUnique(trimmed, { excludeShamoshId });
  return { plain: trimmed, hash: await bcrypt.hash(trimmed, 10) };
}

/** A rabbi (main or additional) sees and manages only their own shamoshim. Legacy-mode tokens see all shamoshim in the org. */
function rabbiOwnedWhere(req: Request): { organizationId: string; rabbiId?: string } {
  const a = authOf(req);
  const where: { organizationId: string; rabbiId?: string } = {
    organizationId: a.organizationId,
  };
  if (a.rabbiId) where.rabbiId = a.rabbiId;
  return where;
}

rabbiRouter.get("/shamoshim", requireRabbiNotShamosh, async (req, res) => {
  const where = rabbiOwnedWhere(req);
...
rabbiRouter.post("/shamoshim", requireRabbiNotShamosh, async (req, res) => {
  ...
  if (!a.rabbiId) {
    res.status(400).json({
      error: "Legacy rabbi sessions cannot manage shamoshim. Sign in with your rabbi password.",
    });
    return;
  }
  ...
  try {
    const pw = await resolveShamoshPassword(parsed.data);
    const created = await prisma.shamosh.create({
      data: {
        organizationId: oid,
        rabbiId: a.rabbiId,
        name: parsed.data.name.trim(),
        phone: parsed.data.phone == null ? null : parsed.data.phone || null,
        email: parsed.data.email == null ? null : parsed.data.email || null,
    ...
    res
      .status(400)
      .json({ error: e instanceof Error ? e.message : "Create failed" });
  }
});
...
rabbiRouter.patch("/shamoshim/:id", requireRabbiNotShamosh, async (req, res) => {
  ...
  const existing = await prisma.shamosh.findFirst({
    where: { ...where, id: String(req.params.id) },
    select: { id: true },
  });
  if (!existing) {
  ...
  try {
    const pw = await resolveShamoshPassword(parsed.data, existing.id);
